 ...
 * users across the N apps? Signature int getBillionUsersDay(float[] growthRates) Input 1.0 <
 * growthRate < 2.0 for all growth rates 1 <= N <= 1,000 Output Return the number of full days it
 * will take before we have a total of 1 billion users across all N apps. Example 1 growthRates =
 * [1.5] output = 52 Example 2 growthRates = [1.1, 1.2, 1.3] output = 79 Example 3 growthRates =
 * [1.01, 1.02] output = 1047
 */
    ...
    int getBillionUsersDay(float[] growthRates) {
      ...
      while(total <= 1000000000){
        ...
        for(int i=0; i<growthRates.length; i++){
          total = power(growthRates[i], days);
        }
